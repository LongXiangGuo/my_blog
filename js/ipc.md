...
想象一下：两段程序运行在同一台机器的同一块硬件上，却像隔着一道无法逾越的叹息之墙——一个进程无法直接读取另一个进程的内存，无法随意抢占对方的CPU时间，更无法擅自调用对方的文件句柄。这种**强制隔离**，是操作系统为现代计算世界筑起的第一道安全防线。

然而，绝对的隔离意味着绝对的孤岛。现实中的软件必须协作：浏览器需要与插件通信，微服务需要交换数据，游戏客户端需要与服务器同步状态。于是，操作系统不得不在这堵墙上凿开一条条“受控通道”——这便是**进程间通信（IPC）** 诞生的根本原因。

但通道从来不是免费的。每一次跨进程的数据搬运，都伴随着上下文切换、CPU拷贝，甚至是内核协议栈的层层封装。选错了通道，高频视频流会被双拷贝拖垮；用错了模型，高并发网关会因阻塞等待而CPU空转。更危险的是，通道本身不校验内容——一个畸形数据包就能通过共享内存击穿整个主进程。**隔离性（安全）与协作性（效率）的博弈**，贯穿了从操作系统内核到上层应用框架的每一层设计。

理解这场博弈，你将获得四种核心能力：

- **守住安全底线**——明白为什么Electron必须关闭`nodeIntegration`，为什么IPC数据必须白名单校验；
- **驾驭极致性能**——看清`零拷贝`与`双拷贝`的数量级差异，吃透`epoll`回调为何比轮询高效数个量级；
- **精准架构选型**——在共享内存、Unix Domain Socket、消息队列之间做出数学最优解；
- **透视现代框架**——看穿Node.js事件循环、Chrome多进程架构、Docker容器隔离背后的统一底层逻辑。

这正是区分“API调用者”与“系统架构师”的分水岭。

本文将从**宏观进程边界**出发，逐层深入到**内核Socket实现（sk_buff、等待队列、epoll）**，最终下沉到**V8引擎的上下文隔离（Context Isolation）**，为你呈现一份从OS到应用层的全链路底层视图。读懂它，你就握住了操作系统调度的“脉搏”。

## 一：Process Model（进程模型）—— 宏观边界
...
## 四：Context Isolation（上下文隔离）—— Web/Electron 层的微观边界
